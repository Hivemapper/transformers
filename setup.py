...
7. Check that everything looks correct by uploading the package to the pypi test server:
   ...
   pip install -i https://testpypi.python.org/pypi transformers_45

   Check you can run the following commands:
   python -c "from transformers_45 import pipeline; classifier = pipeline('text-classification'); print(classifier('What a nice release'))"
   python -c "from transformers_45 import *"
...
# Remove stale transformers_45.egg-info directory to avoid https://github.com/pypa/pip/issues/5466
stale_egg_info = Path(__file__).parent / "transformers_45.egg-info"
if stale_egg_info.exists():
    print(
        (
            "Warning: {} exists.\n\n"
            "If you recently updated transformers_45 to 3.0 or later, this is expected,\n"
            "but it may prevent transformers_45 from installing in editable mode.\n\n"
...
# 2. once modified, run: `make deps_table_update` to update src/transformers_45/dependency_versions_table.py
_deps = [
    "Pillow>=10.0.1,<=15.0",
    "accelerate>=0.26.0",
...
# since we save this data in src/transformers_45/dependency_versions_table.py it can be easily accessed from
# anywhere. If you need to quickly access the data from this table in a shell, you can do so easily with:
#
# python -c 'import sys; from transformers_45.dependency_versions_table import deps; \
# print(" ".join([ deps[x] for x in sys.argv[1:]]))' tokenizers datasets
#
# Just pass the desired package names to that script as it's shown with 2 packages above.
#
# If transformers_45 is not yet installed and the work is done from the cloned repo remember to add `PYTHONPATH=src` to the script above
#
# You can then feed this for example to `pip`:
#
# pip install -U $(python -c 'import sys; from transformers_45.dependency_versions_table import deps; \
# print(" ".join([deps[x] for x in sys.argv[1:]]))' tokenizers datasets)
#
...
class DepsTableUpdateCommand(Command):
    ...
    user_options = [
        ...
        ("dep-table-update", None, "updates src/transformers_45/dependency_versions_table.py"),
    ]
    ...
    def run(self):
        ...
        target = "src/transformers_45/dependency_versions_table.py"
...
# when modifying the following list, make sure to update src/transformers_45/dependency_versions_check.py
install_requires = [
...
setup(
    name="transformers_45",
    version="4.45.2",  # expected format is one of x.y.z.dev0, or x.y.z.rc1 or x.y.z (no to dashes, yes to dots)
    author="The Hugging Face team (past and future) with the help of all our contributors (https://github.com/huggingface/transformers_45/graphs/contributors)",
    author_email="transformers_45@huggingface.co",
    ...
    url="https://github.com/huggingface/transformers_45",
    package_dir={"": "src"},
    ...
    entry_points={"console_scripts": ["transformers_45-cli=transformers_45.commands.transformers_45_cli:main"]},
